...
const Login = () => {
  ...
  const validate = (name, value) => {
    ...
    if (name === 'email' && !/^[a-z0-9.]+@gmail\.com$/.test(val)) {
      return 'Email must be lowercase and numbers no spaces, and end with @gmail.com';
    }
    if (
      name === 'password' &&
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate('/dashboard'); // fetch transactions in dashboard from backend
    } catch (err) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-linear-to-br from-blue-400 via-blue-500 to-cyan-400 p-4">
      <div className="w-full max-w-md bg-white rounded-4xl shadow-2xl p-10">
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          ...
          <button className="w-full bg-linear-to-r from-indigo-500 to-cyan-400 text-white py-3.5 rounded-xl font-bold text-lg shadow-lg hover:opacity-90 transition-opacity">
            Login
          </button>
        </form>
